...
const renderStatus = ({ numPassingTests, numFailingTests, numPendingTests, numTodoTests, testExecError }) => {
  let tagsInfo
  if (testExecError) {
    tagsInfo = (
...
const getColumns = rootDir => [
  ...
  },
  {
    ...
    onFilter: (value, { numFailingTests, numPendingTests, testExecError, numTodoTests }) => {
      switch (value) {
        case 'passed':
  ...
  },
  {
    ...
    title: 'Action',
    key: 'operation',
    render: ({ failureMessage, timelineTraceLink }) => (
      <ErrorButton failureMessage={failureMessage} timelineTraceLink={timelineTraceLink} />
    ),
  },
]
  ...
  <Consumer>
    {({ expand, toggleExpand }) => (
      <Table
        ...
        expandedRowRender={({ testResults, timelineTraceLink }) => (
          <DetailTable data={testResults.map(testResult => ({ ...testResult, timelineTraceLink }))} />
        )}
